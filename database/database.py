...
async def start_sqlite():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS planed_sings(id INTEGER PRIMARY KEY, data DATE,
                    time TIME, category TEXT, client_id INT, client_nickname TEXT,
...
async def create_table_user_history(message: Message):
    ...
    cursor.execute(f"""CREATE TABLE IF NOT EXISTS user_{message.from_user.id}(date DATE, time TIME, category TEXT)""")
    connect.commit()